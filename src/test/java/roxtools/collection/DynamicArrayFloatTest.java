...
public class DynamicArrayFloatTest {

    static Stream<Arguments> dynamicArrayFloatCreation() {
        return Stream.of(
                arguments(3, 100),
                arguments(3, 1234),
                arguments(111, 1234)
    ...
    @MethodSource
    @ParameterizedTest
    void dynamicArrayFloatCreation(int blockSize, int totalValues) {
        final var multiplier = 10;

        var dynamicArray = new DynamicArrayFloat(blockSize, blockSize);
        ...
        for (var i = 0 ; i < 100 ; i++) {
            ...
            assertEquals(i * multiplier, value,
                    "Float at index [" + i + "] doesn't match expected value");
        }
