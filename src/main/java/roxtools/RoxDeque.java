...
final public class RoxDeque<E> extends AbstractCollection<E> implements Deque<E>, List<E>,
	 ...
	 */
	public RoxDeque(int numElements) {
	 ...
	 */
	public RoxDeque(Collection<? extends E> c) {
	...
	public void transferTo(RoxDeque<E> other) {
		transferTo(other, size()) ;
	}
	
	public void transferTo(RoxDeque<E> other, int length) {
		if (length > size()) throw new ArrayIndexOutOfBoundsException() ;
		
		for (int i = 0; i < length; i++) {
			int h = head;
			
			other.addLast( elements[h] ) ;
	...
	public void transferTo(E[] array, int length) {
		if (length > size()) throw new ArrayIndexOutOfBoundsException() ;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public RoxDeque<E> clone() {
		try {
			RoxDeque<E> result = (RoxDeque<E>) super.clone();
	...
	private int getIdxPos(int idx) {
		if (idx >= size()) throw new ArrayIndexOutOfBoundsException(idx) ;
		return (head + idx) % elements.length ;
	}
	
	private int getIdxPosNoBoundsCheck(int idx) {
		return (head + idx) % elements.length ;
	}
	
	@SuppressWarnings("unused")
	private int getPosIdx(int pos) {
		int idx = pos - head ;
	...
	public E get(int idx) {
		return elements[ getIdxPos(idx) ] ;
	}
	
	public E getNoBoundsCheck(int idx) {
		return elements[ getIdxPosNoBoundsCheck(idx) ] ;
	}
	...
	public void add(int idx, E elem) {
		if (idx == 0) {
			...
			if (idx == sz) {
			...
			else if (head < tail && tail < elements.length-1) {
				int pos = head+idx ;
				System.arraycopy(elements, pos, elements, pos + 1, sz - idx);
				elements[pos] = elem ;
				tail = head+sz+1 ;
			}
			else {
				...
				if (head < tail) {
					System.arraycopy(elements, head, elems, 0, idx);
					System.arraycopy(elements, head+idx, elems, idx+1, sz-idx);
					elems[idx] = elem ;
				}
				else {
					int headPortionLen = n - head;
					
					if (idx < headPortionLen) {
						System.arraycopy(elements, head, elems, 0, idx);
						System.arraycopy(elements, head+idx, elems, idx+1, headPortionLen-idx);
						System.arraycopy(elements, 0, elems, headPortionLen+1, tail);
					}
					else { 
						int i = idx-headPortionLen ;
						System.arraycopy(elements, head, elems, 0, headPortionLen);
						System.arraycopy(elements, 0, elems, headPortionLen, i);
						System.arraycopy(elements, i, elems, headPortionLen+i+1, tail-i);
					}
	...
	public int getInsertSortedIndex(E elem, Comparator<E> comparator) {
		...
		while (low <= high) {
		    int mid = (low + high) >>> 1;
		    E midVal = (E) elements[ getIdxPosNoBoundsCheck(mid) ] ;
	...
	public E removeNoBoundsCheck(int idx) {
		if (idx == 0) return removeFirst() ;
		
		int pos = getIdxPosNoBoundsCheck(idx) ;
		E elem = elements[pos] ;
		delete(pos) ;
		return elem ;
	}
	...
	public int indexOf(Object o) {
		...
		for (int i = 0; i < forSz; i++) {
			E e = elements[ getIdxPosNoBoundsCheck(i) ] ;
	...
	private class DeqListIterator extends DeqIterator implements ListIterator<E> {
		public DeqListIterator(int idx) {
			super() ;
			this.cursor = getIdxPosNoBoundsCheck(idx) ;
		}

		@Override
		public void add(E e) {
			// Cursor already pointing to the next element
			RoxDeque.this.add(getIdxPosNoBoundsCheck(this.cursor) , e) ;
		}

		@Override
		public boolean hasPrevious() {
			return this.lastRet != RoxDeque.this.head ;
		}

		@Override
		public int nextIndex() {
			return getIdxPosNoBoundsCheck(this.lastRet) ;
		}
		...
		public int previousIndex() {
			int idx = getIdxPosNoBoundsCheck(this.lastRet)-1 ;
			if (idx < 0) return -1 ;
			return idx ;
		}
		...
		@Override
		public void set(E e) {
			RoxDeque.this.elements[this.lastRet] = e ;
		}
	}

	@Override
	public E set(int index, E element) {
		int pos = getIdxPos(index) ;
		E prev = elements[pos] ;
